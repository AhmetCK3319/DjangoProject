...
def store(request,category_slug = None):
    ...
    context = {
    'products'      : page_products, # context de gönder
...
def product_detail(request,category_slug,product_slug):
    try:
        ...
        # Cart ın id si =  _cart_id(request)
        # exists = boolean döner
    ...
    if request.user.is_authenticated:
        ...
        orderproduct=None        
    # bütün mesajlar 
    all_reviews=ReviewRating.objects.filter(product_id=single_product.id)
    # all galery
...
def search(request):
    if request.method == 'GET':
        ...
        if keyword:
            ...
            Q(product_name__icontains = keyword)).distinct()
            product_count  = products.count()
